...
## Agent 工作树映射（2026-09-09 起：单工作树协作）

> **分支与工作树已全部收敛**：仅保留 `main` 分支（本地 + gitee + github，均= `981daf3`）。原 5 个 `ZX466/*` 分支、本地 agent 分支与 5 个 agent worktree 已删除（2026-09-09）。
> 各 Agent 的记忆已存档于主工作树：`.orca/agent-memory-pi.md`、`.orca/agent-memory-codex.md`、`AGENTS.md`（opencode 会话记忆）。
> 后续协作直接在 `main` 上按能力域分工：任务由 Claude 写入主工作树 `.orca/talking.txt`，各 agent 会话读板认领；产出由 Claude 汇总入 main。
...
| 2026-09-09 | worktree 全部删除，收敛为单工作树（main）协作模式 | 用户 |
